...
// variables -- a fine alpha-variant of the term, and a reading a language may want. It
// puts a duty on a rewrite: a right-hand side that rebinds that slot over a variable
// matched OUTSIDE the binder captures it under that reading, so the rewrite has to say
// the two do not alias. The nested matcher never offers the reading, its bound slots
// staying injective, which is why such a rewrite could lack the guard there.
//
...
// captured, a closed term, and the class stops depending on `$2`. `!slot_free_in(y, e)`
// is the guard that excludes the reading. Left unguarded on the paper's array language,
// `let-lam-diff` does this at zero parameters six rounds in, after which every class is
// one class.
fn multipat_rhs_binder_over_outside_variable_needs_a_guard() {
    ...
    // nested matcher: one reading, the sound result, guard or no guard
    let mut eg: EGraph<Arith2> = EGraph::new(());
    ...
    // multipattern matcher: the same rule flattened has TWO readings, and applied
    // without a guard the second captures
    ...
    let captured_id = eg.add_expr(RecExpr::parse(captured).unwrap());
    assert!(eg.eq(&start, &captured_id), "unguarded, the aliasing reading captures ?e");

    // the guard the rewrite owes: `$y` is not free in `?e`
    let mut eg: EGraph<Arith2> = EGraph::new(());
    let start = eg.add_expr(RecExpr::parse(term).unwrap());
    for s in multi_ematch(&pat, &eg) {
        if !s["e"].slots().contains(&Slot::named("y")) {
            eg.union_instantiations(&from, &to, &s, None);
        }
    }
    let sound_id = eg.add_expr(RecExpr::parse(sound).unwrap());
